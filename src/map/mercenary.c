...
void merc_contract_stop(struct mercenary_data *md)
{
	...
	if(md->contract_timer != INVALID_TIMER)
		delete_timer(md->contract_timer, mercenary->contract_end_timer);
	md->contract_timer = INVALID_TIMER;
}
...
{
	if(md->contract_timer == INVALID_TIMER)
		md->contract_timer = add_timer(gettick() + md->mercenary.life_time, mercenary->contract_end_timer, md->master->bl.id, 0);
...
int read_mercenarydb(void)
{
	...
	sv_readsqldb(get_database_name(17), NULL, 26, MAX_MERCENARY_CLASS, mercenary->read_db_sub);
...
{
	sv_readsqldb(get_database_name(18), NULL, 3, -1, mercenary->read_skill_db_sub);
	return 0;
}
...
void do_init_mercenary(void) {
	...
	add_timer_func_list(mercenary->contract_end_timer, "merc_contract_end_timer");
}
