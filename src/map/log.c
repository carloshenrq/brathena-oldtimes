...
void log_pick_sub_sql(int id, int16 m, e_log_pick_type type, int amount, struct item* itm, struct item_data *data) {
	if( SQL_ERROR == Sql_Query(logs->mysql_handle,
	    ...
	    map[m].name?map[m].name:"", itm->unique_id)
	) {
...
void log_zeny_sub_sql(struct map_session_data* sd, e_log_pick_type type, struct map_session_data* src_sd, int amount) {
	if(SQL_ERROR == Sql_Query(logs->mysql_handle, LOG_QUERY " INTO `%s` (`time`, `char_id`, `src_id`, `type`, `amount`, `map`) VALUES (NOW(), '%d', '%d', '%c', '%d', '%s')",
...
void log_mvpdrop_sub_sql(struct map_session_data* sd, int monster_id, int* log_mvp) {
	if(SQL_ERROR == Sql_Query(logs->mysql_handle, LOG_QUERY " INTO `%s` (`mvp_date`, `kill_char_id`, `monster_id`, `prize`, `mvpexp`, `map`) VALUES (NOW(), '%d', '%d', '%d', '%d', '%s') ",
